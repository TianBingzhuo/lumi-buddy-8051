...
这个项目的起点是把实验八从传统的字符式温度时钟改造成资源受限条件下的图形 HUD。核心约束包括 8051 内部 RAM 容量有限、LCD12864 写入速度较慢，以及 DS1302 和 DS18B20 对时序的严格要求。因此，架构目标不是继续叠加功能，而是在显示稳定的前提下组织好界面、时钟和预留传感器链路。
